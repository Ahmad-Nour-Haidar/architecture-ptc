...
class ReplayBoxRemoteDataSource {
  ...
  Future<BaseModel> uploadReplayFiles({
  ...
  Future<BaseModel> createReplayBox({
    ...
    return BaseModel(
        data: null, message: response['message'] ?? "Create replay successful");
  }

  Future<BaseModel> getReplayBoxById({required int replayBoxId}) async {
    final response = await _apiServices
        .get('${AppUrl.replayMailboxById}$replayBoxId', hasToken: true);
    return BaseModel.fromJson(
      response,
  ...
  Future<BaseModel> getReplayBoxes({required int? page}) async {
  ...
  Future<BaseModel> deleteReplayBox({required int replayBoxId}) async {
    final response = await _apiServices
        .delete('${AppUrl.replayMailboxById}/$replayBoxId', hasToken: true);
    return BaseModel(
        data: null, message: response['message'] ?? "Delete replay successful");
  }
  ...
  Future<BaseModel> updateReplayBox({
    ...
    final response = await _apiServices.put(
        '${AppUrl.replayMailboxById}/$replayBoxId',
        body: body,
        hasToken: true);
    return BaseModel(
        data: null, message: response['message'] ?? "Update replay successful");
  }
}
