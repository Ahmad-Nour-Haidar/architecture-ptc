...
class ReplayBoxRepository {
  ...
  Future<ApiResponse<BaseModel>> createReplayBox({
    ...
    try {
      final response = await _remoteDataSource.createReplayBox(
  ...
  Future<ApiResponse<BaseModel>> getReplayBoxById(
      {required int replayBoxId}) async {
    try {
      final response =
          await _remoteDataSource.getReplayBoxById(replayBoxId: replayBoxId);
      return ApiResponse.success(response);
    } catch (error) {
      return ApiResponse.failure(
        NetworkExceptions.getException(error),
  ...
  Future<ApiResponse<BaseModel>> uploadReplayFiles({
  ...
  Future<ApiResponse<BaseModel>> updateReplayBox(
      ...
      required int replayBoxId}) async {
    try {
      final response = await _remoteDataSource.updateReplayBox(
          replayBox: replayBox, replayBoxId: replayBoxId);
      return ApiResponse.success(response);
    } catch (error) {
      return ApiResponse.failure(
        NetworkExceptions.getException(error),
  ...
  Future<ApiResponse<BaseModel>> deleteReplayBox(
    ...
    try {
      final response =
          await _remoteDataSource.deleteReplayBox(replayBoxId: replayBoxId);
      return ApiResponse.success(response);
    } catch (error) {
      return ApiResponse.failure(
        NetworkExceptions.getException(error),
